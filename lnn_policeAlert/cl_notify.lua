...
function SendNotification(options)
    ...
    if IsPedInAnyVehicle(PlayerPedId(), true) then
        local options = {
			...
			color = options.color or "rgb(0,0,0,0.4)",
...
RegisterNetEvent("lonenoti:SendNotification")
AddEventHandler("lonenoti:SendNotification", function(options)
    SendNotification(options)
end)

RegisterNetEvent("lonenoti:SetQueueMax")
AddEventHandler("lonenoti:SetQueueMax", function(queue, max)
    SetQueueMax(queue, max)
end)

--[[RegisterNetEvent("chatMessage")
AddEventHandler("chatMessage", function(author, color, text)
    TriggerEvent("lonenoti:SendNotification", {text = "<span style='font-weight: 900'>" .. text .. "</span>",
        layout = "centerLeft",
        timeout = 2000,
        progressBar = false,
        type = "error",
